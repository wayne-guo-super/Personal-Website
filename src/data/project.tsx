// data/projects.ts
export const projects = [
    {
        title: "Portfolio Page",
        description: "Personal website.",
        skills: ["Next.JS", "Three.js", "React Three Fiber", "R2 Storage"],
        image: "/project-images/portfolio.webp",
        link: "./",
    },
    {
        title: "ACC Simulator",
        description: "Optimizing the PID controller parameters using the vehicle dynamics model to achieve peak ACC performance.",
        skills: ["C++", "NumPy", "PID Controller"],
        image: "/project-images/acc-simulator.webp",
        link: "/source/ACC_simulation.mov",
    },
    {
        title: "OpenGL Showcase",
        description: "A rendering framework leveraging OpenGL to generate immersive 3D scenes with models and shaders.",
        skills: ["C++", "OpenGL", "shaders"],
        image: "/project-images/openGL-showcase.webp",
        link: "/source/at_disco_dyn.mov",
    },
    {
        title: "Mentor Hub",
        description: "A platform to facilitate student collaboration through study group matching and event participation.",
        skills: ["React", "HCD"],
        image: "/project-images/mentorHub.webp",
        link: "https://mentorhub.amrthabit.com",
    },
    {
        title: "War-Map",
        description: "[GDSC 2024] A visualization tool to demonstrate the mortality demographic caused by regional conflict.",
        skills: ["React", "Google Maps API"],
        image: "/project-images/war-map.webp",
        link: "https://devpost.com/software/mapping-humanity-israel-palestine-war?_gl=1*1x574fq*_gcl_au*NDY5NDE1NjUzLjE3NDI1MzExMTU.*_ga*MTkzNzE3MTkuMTc0MjUzMTExNQ..*_ga_0YHJK3Y10M*MTc0Mjg1MzI2Mi4zLjEuMTc0Mjg1MzI2Ny4wLjAuMA..",
    },
];